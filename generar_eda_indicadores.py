...
<section id="ipm" class="panel" hidden><div id="ipm-kpis" class="grid kpis"></div><div class="card grid filters" style="grid-template-columns:repeat(3,minmax(180px,1fr))"><div class="field"><label>Resultado para contrastar con IPM</label><select id="ipm-outcome"></select></div><div class="field"><label>Departamento</label><select id="ipm-dept"></select></div><div class="field"><label>Criterio de priorización</label><select id="ipm-score-mode"><option value="both">IPM + resultado</option><option value="ipm">Solo IPM</option><option value="outcome">Solo resultado</option></select></div></div><div class="grid two"><div class="card"><h2>Vulnerabilidad previa vs. necesidad/afectación</h2><p id="scatter-note" class="muted"></p><div id="scatter"></div></div><div class="card"><h2>Municipios prioritarios</h2><p class="muted">Puntaje = promedio de percentiles dentro del universo común. Es una herramienta exploratoria, no un nuevo índice oficial.</p><div id="priority" class="scroll"></div></div></div><div class="card trend"><div class="section-title"><div><h2>Evolución</h2><p>Mediana municipal del indicador por fecha de corte.</p></div></div><div id="trend"></div></div></section>
...
function renderSummary(){let s=DATA.summary;$('#summary-kpis').innerHTML=[['Observaciones',s.filas.toLocaleString('es-CO'),'filas crudas'],['Indicadores',s.indicadores,'sin derivados'],['Fuentes',s.fuentes,'coberturas distintas'],['Municipios',s.municipios,'pares departamento–municipio'],['Municipios con IPM',s.ipm_n,'zona UNDP‑RÁPIDA'],['Cortes históricos',s.fechas.length,s.fechas.length<2?'evolución aún no disponible':'serie disponible']].map(x=>`<div class="card kpi"><div class="label">${x[0]}</div><div class="value">${x[1]}</div><div class="sub">${x[2]}</div></div>`).join('');let dims=uniq(DATA.rows.map(r=>r.dim));let rows=dims.map(d=>{let z=DATA.rows.filter(r=>r.dim===d),t=new Set(z.map(territory));return `<tr><td><b>${d}</b></td><td class="r">${z.length.toLocaleString('es-CO')}</td><td class="r">${t.size.toLocaleString('es-CO')}</td><td class="r">${new Set(z.map(r=>r.id)).size}</td></tr>`});$('#dimension-table').innerHTML=table(['Dimensión','Observaciones','Territorios','Indicadores'],rows);$('#headline-findings').innerHTML=DATA.audit.findings.slice(0,4).map(f=>`<div class="finding"><span class="badge ${f.severity}">${f.severity}</span><h3>${f.title}</h3><p>${f.detail}</p></div>`).join('')}

function indicatorsFor(level,dim){return uniq(DATA.rows.filter(r=>r.lv===level&&(!dim||r.dim===dim)).map(r=>r.id))}
...
function renderRanking(){let level=$('#rank-level').value,id=$('#rank-indicator').value,dep=$('#rank-dept').value,search=$('#rank-search').value.toLowerCase(),order=$('#rank-order').value;let z=DATA.rows.filter(r=>r.lv===level&&r.id===id&&(!dep||r.d===dep)&&(!search||territory(r).toLowerCase().includes(search)));z.sort((a,b)=>(order==='desc'?b.v-a.v:a.v-b.v)||territory(a).localeCompare(territory(b),'es'));let meta=z[0]||DATA.rows.find(r=>r.id===id),vals=z.map(r=>r.v),max=Math.max(...vals,1);$('#rank-title').textContent=meta?meta.i:'Ranking';$('#rank-sub').textContent=meta?`${meta.u} · ${meta.f} · ${z.length} territorios con dato`:'';$('#ranking').innerHTML=z.length?z.slice(0,40).map((r,i)=>`<div class="rank-row"><span class="rank-pos">${i+1}</span><span>${territory(r)}</span><div class="bar"><div class="fill" style="width:${100*r.v/max}%"></div></div><span class="num">${fmt(r.v,r.u)}</span></div>`).join(''):'<div class="empty">Sin datos para estos filtros.</div>';$('#rank-stats').innerHTML=vals.length?table(['Estadístico','Valor'],[['Mínimo',fmt(Math.min(...vals),meta.u)],['Mediana',fmt(q(vals,.5),meta.u)],['Promedio',fmt(vals.reduce((a,b)=>a+b,0)/vals.length,meta.u)],['Máximo',fmt(Math.max(...vals),meta.u)],['Con dato',vals.length.toLocaleString('es-CO')]].map(x=>`<tr><td>${x[0]}</td><td class="r"><b>${x[1]}</b></td></tr>`)):'<p class="muted">Sin distribución.</p>'}
function initRanking(){let dims=uniq(DATA.rows.filter(r=>r.lv==='municipal').map(r=>r.dim));opts($('#rank-dim'),dims);opts($('#rank-dept'),uniq(DATA.rows.filter(r=>r.lv==='municipal').map(r=>r.d)),'Todos');['#rank-level','#rank-dim'].forEach(s=>$(s).onchange=()=>{if(s==='#rank-level'){let lv=$('#rank-level').value;opts($('#rank-dim'),uniq(DATA.rows.filter(r=>r.lv===lv).map(r=>r.dim)));opts($('#rank-dept'),uniq(DATA.rows.filter(r=>r.lv===lv).map(r=>r.d)),'Todos')}setRankIndicators()});['#rank-indicator','#rank-dept','#rank-order'].forEach(s=>$(s).onchange=renderRanking);$('#rank-search').oninput=renderRanking;setRankIndicators()}

function initProfile(){let territories=uniq(DATA.rows.map(territory));opts($('#profile-territory'),territories);$('#profile-territory').onchange=renderProfile;renderProfile()}
function renderProfile(){let t=$('#profile-territory').value,z=DATA.rows.filter(r=>territory(r)===t);z.sort((a,b)=>a.dim.localeCompare(b.dim,'es')||a.i.localeCompare(b.i,'es'));$('#profile').innerHTML=z.length?z.map(r=>{let peers=DATA.rows.filter(x=>x.lv===r.lv&&x.id===r.id).map(x=>x.v),p=pct(peers,r.v);return `<div class="metric-row"><div><b>${r.i}</b><div class="muted">${r.dim} · ${r.f}</div></div><div class="num">${fmt(r.v,r.u)}</div><div class="bar"><div class="fill" style="width:${p}%"></div></div><div class="pct">P${Math.round(p)}</div></div>`}).join(''):'<div class="empty">Sin indicadores.</div>'}

function joinIpm(outcome,dep){let ipm=new Map(DATA.rows.filter(r=>r.id==='undp_rapida_mpi'&&(!dep||r.d===dep)).map(r=>[`${r.d}|${r.m}`,r]));return DATA.rows.filter(r=>r.id===outcome&&r.lv==='municipal'&&(!dep||r.d===dep)).map(r=>({x:ipm.get(`${r.d}|${r.m}`),y:r})).filter(o=>o.x)}
function initIpm(){let candidates=uniq(DATA.rows.filter(r=>r.lv==='municipal'&&r.id!=='undp_rapida_mpi').map(r=>r.id));$('#ipm-outcome').innerHTML=candidates.map(id=>{let r=DATA.rows.find(x=>x.id===id);return `<option value="${id}">${r.i} · ${r.f}</option>`}).join('');let preferred=['undp_rapida_recovery_needs','undp_rapida_pop_imp','undp_rapida_econ_dmg_total_cop'];$('#ipm-outcome').value=preferred.find(x=>candidates.includes(x))||candidates[0];opts($('#ipm-dept'),uniq(DATA.rows.filter(r=>r.id==='undp_rapida_mpi').map(r=>r.d)),'Todos');['#ipm-outcome','#ipm-dept','#ipm-score-mode'].forEach(s=>$(s).onchange=renderIpm);renderIpm()}
function renderIpm(){let id=$('#ipm-outcome').value,dep=$('#ipm-dept').value,mode=$('#ipm-score-mode').value,pairs=joinIpm(id,dep),xs=pairs.map(o=>o.x.v),ys=pairs.map(o=>o.y.v),xq=q(xs,.75),yq=q(ys,.75),meta=pairs[0]?.y||DATA.rows.find(r=>r.id===id);let scored=pairs.map(o=>{let px=pct(xs,o.x.v),py=pct(ys,o.y.v),score=mode==='ipm'?px:mode==='outcome'?py:(px+py)/2;return {...o,px,py,score,high:o.x.v>=xq&&o.y.v>=yq}}).sort((a,b)=>b.score-a.score);let high=scored.filter(x=>x.high).length;$('#ipm-kpis').innerHTML=[['Municipios comparables',pairs.length,'con IPM y resultado'],['IPM mediano',fmt(q(xs,.5)),'línea base'],['Umbral IPM alto',fmt(xq),'cuartil superior'],['Resultado mediano',fmt(q(ys,.5),meta?.u),'universo común'],['Alta vulnerabilidad + necesidad',high,'cuadrante superior'],['Departamentos',new Set(pairs.map(o=>o.x.d)).size,'con comparación']].map(x=>`<div class="card kpi"><div class="label">${x[0]}</div><div class="value">${x[1]}</div><div class="sub">${x[2]}</div></div>`).join('');$('#scatter-note').textContent=meta?`${meta.i} (${meta.u}) frente a IPM. Líneas: percentil 75 de cada eje.`:'';renderScatter(scored,xq,yq,meta);$('#priority').innerHTML=table(['#','Municipio','IPM','Resultado','Puntaje'],scored.slice(0,40).map((o,i)=>`<tr><td>${i+1}</td><td><b>${o.x.m}</b><br><span class="muted">${o.x.d}</span></td><td class="r">${fmt(o.x.v)}</td><td class="r">${fmt(o.y.v,o.y.u)}</td><td class="r"><b>${o.score.toFixed(0)}</b>${o.high?' <span class="badge alta">alto-alto</span>':''}</td></tr>`));renderTrend(id,dep,meta)}
...
renderSummary();initRanking();initProfile();initIpm();renderAudit();
</script></body></html>'''
