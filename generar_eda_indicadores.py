...
function setRankIndicators(preferredId=''){let level=$('#rank-level').value,dim=$('#rank-dim').value,ids=indicatorsFor(level,dim);let prev=preferredId||$('#rank-indicator').value;$('#rank-indicator').innerHTML=ids.map(id=>{let r=DATA.rows.find(x=>x.id===id);return `<option value="${id}">${r.i} · ${r.f}</option>`}).join('');if(ids.includes(prev))$('#rank-indicator').value=prev;renderRanking()}
...
function initRanking(preferredId=''){let base=viewRows(),previousDept=$('#rank-dept').value,selected=base.find(r=>r.id===preferredId),level=selected?.lv||$('#rank-level').value||'municipal';$('#rank-level').value=level;let dims=uniq(base.filter(r=>r.lv===level).map(r=>r.dim));opts($('#rank-dim'),dims);if(selected&&dims.includes(selected.dim))$('#rank-dim').value=selected.dim;let depts=uniq(base.filter(r=>r.lv===level).map(r=>r.d));opts($('#rank-dept'),depts,'Todos');if(depts.includes(previousDept))$('#rank-dept').value=previousDept;['#rank-level','#rank-dim'].forEach(s=>$(s).onchange=()=>{if(s==='#rank-level'){let lv=$('#rank-level').value;opts($('#rank-dim'),uniq(viewRows().filter(r=>r.lv===lv).map(r=>r.dim)));opts($('#rank-dept'),uniq(viewRows().filter(r=>r.lv===lv).map(r=>r.d)),'Todos')}setRankIndicators()});['#rank-indicator','#rank-dept','#rank-order'].forEach(s=>$(s).onchange=renderRanking);$('#rank-search').oninput=renderRanking;setRankIndicators(preferredId)}
...
function initIpm(preferredId=''){let base=viewRows(),previousId=preferredId||$('#ipm-outcome').value,previousDept=$('#ipm-dept').value,candidates=uniq(base.filter(r=>r.lv==='municipal'&&r.id!=='undp_rapida_mpi').map(r=>r.id));$('#ipm-outcome').innerHTML=candidates.map(id=>{let r=base.find(x=>x.id===id);return `<option value="${id}">${r.i} · ${r.f}</option>`}).join('');let preferred=['undp_rapida_recovery_needs','undp_rapida_pop_imp','undp_rapida_econ_dmg_total_cop'];$('#ipm-outcome').value=candidates.includes(previousId)?previousId:(preferred.find(x=>candidates.includes(x))||candidates[0]);let depts=uniq(base.filter(r=>r.id==='undp_rapida_mpi').map(r=>r.d));opts($('#ipm-dept'),depts,'Todos');if(depts.includes(previousDept))$('#ipm-dept').value=previousDept;['#ipm-outcome','#ipm-dept'].forEach(s=>$(s).onchange=renderIpm);renderIpm()}
...
function refreshScope(){let rankIndicator=$('#rank-indicator').value,ipmOutcome=$('#ipm-outcome').value,decree=$('#territory-scope').value==='decree';$('#scope-note').textContent=decree?`${DATA.decree_departments.length} departamentos nombrados en el Decreto 1171. Todas las vistas territoriales usan este universo.`:'Inventario completo. Las filas de ExE fuera del Decreto 1171 se muestran como necesidades no atribuidas al sismo.';renderSummary();initRanking(rankIndicator);initProfile();initIpm(ipmOutcome)}
