#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Genera un tablero autocontenido del índice municipal de educación.
...
def build_index(rows):
    municipal = [
        row for row in rows
        if row.get("nivel") == "municipal"
        and row.get("indicador_id") in MUNICIPAL_INDICATORS
    ...
    for row in municipal:
        ...
        cohorts[indicator_id].append(value)
    ...
    profiles = {}
    total_variables = len(MUNICIPAL_INDICATORS)
...
def compact_inventory(rows, decree):
    decree = set(decree)
    result = []
    for row in education_rows(rows):
        value = as_number(row.get("valor"))
...
def build_html(rows):
    payload = json.dumps(build_payload(rows), ensure_ascii=False, separators=(",", ":")).replace("</", "<\\/")
...
<title>Prioridad educativa municipal</title>
<style>
...
<header class="top"><h1>Prioridad educativa municipal</h1><p>Índice de evidencia disponible: ocho variables educativas tienen el mismo peso máximo. El resultado combina intensidad observada y cobertura documental; una posición alta indica mayor prioridad en los datos disponibles, no atribución causal verificada al sismo.</p></header>
<nav class="tabs" aria-label="Vistas"><button class="tab active" data-view="ranking">Ranking municipal</button><button class="tab" data-view="datos">Datos educativos</button><button class="tab" data-view="metodo">Método</button></nav>
...
<section id="ranking" class="view active">
  ...
  <div class="card controls">
    ...
    <label>Buscar municipio<input id="search" type="search" placeholder="Nombre o DIVIPOLA"></label>
  </div>
  <div class="note">PNUD <b>pnud_cedu</b> y UNDP-RÁPIDA <b>undp_rapida_bdg_edu_aff</b> coinciden exactamente en los 258 municipios compartidos. Se mantienen separados porque se pidió igual peso para todos los campos; por eso ese mismo dato publicado por dos canales puede aportar dos veces.</div>
  <div class="grid" style="margin-top:14px">
    <div class="card"><h2>Ranking de prioridad</h2><div class="scroll"><table><thead><tr><th>#</th><th>Municipio</th><th>Departamento</th><th class="num">Índice</th><th class="num">Intensidad</th><th class="num">Cobertura</th></tr></thead><tbody id="rankingRows"></tbody></table></div></div>
    <div><div class="card"><h2>Primeros municipios</h2><div id="bars"></div></div><div class="card profile" id="profile"></div></div>
  </div>
</section>
...
<section id="metodo" class="view"><div class="card method"><h2>Cómo se construye</h2><ol><li>Se usan ocho variables municipales: edificaciones o puntos educativos, costo estimado, sedes, sedes críticas, matrícula y docentes afectados.</li><li>Cada valor se convierte en percentil de 0 a 100 dentro de su propia variable. Así se pueden combinar COP y conteos sin mezclar sus unidades originales.</li><li>Cada variable tiene el mismo peso máximo: 1/8 del total. Si un municipio no tiene una variable, esa ausencia reduce su cobertura y no se presenta como un cero observado.</li><li>La intensidad es el promedio de los percentiles que sí existen. El índice final es intensidad × cobertura, equivalente a sumar los percentiles disponibles y dividir entre ocho.</li><li>El ranking favorece municipios con mayor señal educativa y mayor cantidad de variables observadas. Los valores crudos siguen visibles en el perfil y el inventario.</li></ol><div class="formula">Índice = Σ(percentil de cada variable disponible) ÷ 8<br>Intensidad = Σ(percentiles disponibles) ÷ número de variables disponibles<br>Cobertura = variables disponibles ÷ 8<br>Índice = Intensidad × Cobertura</div><h2 style="margin-top:20px">Qué no afirma</h2><p>El índice mide prioridad según la evidencia reunida, no daño causal confirmado. Fundación ExE incluye necesidades educativas fuera del área del decreto. Además, PNUD y UNDP-RÁPIDA publican el mismo conteo educativo en su cobertura común. El filtro del decreto es administrativo y no prueba afectación municipal.</p></div></section>
</main>
<script id="payload" type="application/json">__PAYLOAD__</script>
<script>
const D=JSON.parse(document.getElementById('payload').textContent), order=D.indicator_order, decree=new Set(D.decree_departments);let selected=null,rawPage=1;const $=id=>document.getElementById(id), esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function init(){
  $('kpis').innerHTML=`<div class="kpi"><strong>${D.summary.municipalities}</strong><span>municipios con al menos un dato</span></div><div class="kpi"><strong>${D.summary.variables}</strong><span>variables con igual peso</span></div><div class="kpi"><strong>${D.summary.complete}</strong><span>municipios con cobertura completa</span></div><div class="kpi"><strong>${Math.round(D.summary.median_coverage*100)}%</strong><span>cobertura mediana</span></div>`;
  ...
  $('indicatorRows').innerHTML=D.indicators.map(x=>`<tr><td>${esc(x.name)}</td><td>${esc(x.level)}</td><td>${esc(x.source)}</td><td class="num">${x.n}</td><td class="num">${x.zeroes}</td><td class="num">${fmt(x.median,x.unit)}</td><td class="num">${fmt(x.max,x.unit)}</td><td>${x.used?'<span class="pill">Índice municipal</span>':'Solo inventario'}</td></tr>`).join('');
  ['scope','department','coverage','search'].forEach(id=>$(id).addEventListener(id==='search'?'input':'change',renderRanking));
  ['rawLevel','rawIndicator','rawSource','rawSearch'].forEach(id=>$(id).addEventListener(id==='rawSearch'?'input':'change',()=>{rawPage=1;renderRaw()}));$('prev').onclick=()=>{rawPage=Math.max(1,rawPage-1);renderRaw()};$('next').onclick=()=>{rawPage++;renderRaw()};renderRanking();renderRaw();
}
...
function renderRanking(){const rows=rankingData();if(!rows.some(x=>x.key===selected))selected=rows[0]?.key||null;$('rankingRows').innerHTML=rows.map((x,i)=>`<tr data-key="${esc(x.key)}" class="${x.key===selected?'selected':''}"><td>${i+1}</td><td><b>${esc(x.municipality)}</b><br><small>${esc(x.code)}</small></td><td>${esc(x.department)}</td><td class="num"><b>${x.score.toFixed(1)}</b></td><td class="num">${x.intensity.toFixed(1)}</td><td class="num">${x.observed}/8</td></tr>`).join('')||'<tr><td colspan="6">No hay municipios con estos filtros.</td></tr>';document.querySelectorAll('#rankingRows tr[data-key]').forEach(tr=>tr.onclick=()=>{selected=tr.dataset.key;renderRanking()});$('bars').innerHTML=rows.slice(0,12).map(x=>`<div class="barrow"><span>${esc(x.municipality)}</span><div class="track"><div class="fill" style="width:${x.score}%"></div></div><b class="num">${x.score.toFixed(1)}</b></div>`).join('')||'<p>Sin datos.</p>';renderProfile(rows.find(x=>x.key===selected))}
function renderProfile(x){if(!x){$('profile').innerHTML='<h2>Perfil municipal</h2><p>Selecciona un municipio.</p>';return}$('profile').innerHTML=`<h2>${esc(x.municipality)}, ${esc(x.department)}</h2><div class="headline">${x.score.toFixed(1)}</div><small>Índice educativo · intensidad ${x.intensity.toFixed(1)} · cobertura ${x.observed}/8</small><div style="margin-top:14px">${order.map(id=>{const v=x.variables[id],meta=D.indicators.find(i=>i.id===id&&i.level==='municipal');return v?`<div class="profile-row"><b>${esc(v.name)}</b><span>${fmt(v.value,v.unit)} · percentil ${v.percentile.toFixed(1)}</span><br><small>${esc(v.source)} · aporte ${(v.percentile/8).toFixed(1)} puntos</small></div>`:`<div class="profile-row missing"><b>${esc(meta?.name||id)}</b>Sin dato · aporte 0 por falta de cobertura</div>`}).join('')}</div>`}
...
def main():
    ...
    print(f"Generado {args.out} con {len(build_index(rows))} municipios y {len(education_rows(rows))} registros educativos.")
