...
(async()=>{
 ...
 try {
  ...
  await page.locator('#priority-dimension').selectOption('educacion');
  assert.match(await page.locator('#priority-note').innerText(),/Puntaje documentado de Educación/);
  assert.match(await page.locator('#matrix tbody tr').first().innerText(),/Puesto en dimensión/);
  assert.equal(await page.locator('#matrix th').nth(1).innerText(),'Educación');
  await page.locator('#priority-order').selectOption('uncertainty');
  assert.match(await page.locator('#priority-note').innerText(),/Puntaje posible de Educación/);
