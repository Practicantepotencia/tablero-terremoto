...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(await page.locator('[data-tab]').allTextContents(),['Prioridades','Comparar municipios','Recuperación RAPIDA','Diagnóstico territorial','Fuentes y método']);
  ...
  const common=await page.locator('[data-compare-geo]').evaluateAll(xs=>xs.map(x=>x.dataset.compareGeo).sort());
  ...
  assert.deepEqual(await page.locator('[data-compare-geo]').evaluateAll(xs=>xs.map(x=>x.dataset.compareGeo).sort()),common);
  assert.match(await page.locator('#comparison-warning').innerText(),/parcial/);
  ...
  await page.locator('#comparison-axis').selectOption('value');
  assert.match(await page.locator('#comparison-kpis').innerText(),/valor RAPIDA/);
  await page.locator('#comparison-mode').selectOption('absolute');
  await page.locator('#comparison-panel').selectOption('available');
  assert.ok(await page.locator('[data-compare-geo]').count()>=common.length);
  await page.locator('#scope').selectOption('all');
  await page.locator('#comparison-axis').selectOption('rank');
