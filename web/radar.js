...
(function(root){
  ...
  function relativeField(f,r){
    const head=`<div class="radar-input"><strong>${esc(f.label)}</strong><div class="small muted">${esc(f.source)} · N comparable = ${f.n}</div><p>Conteo original: ${f.row?fmt(f.row.v):'sin dato'} · Población: ${r.population?fmt(r.population.population):'sin dato'}</p>`;
    if(f.score==null)return head+`<p><strong>Sin tasa calculable, no cero.</strong> Falta un conteo comparable o población válida. Peso interno = ${fmt(f.share)}; aporte desconocido al sector: 0–${fmt(100*f.share)}.</p></div>`;
    return head+`<div class="formula">Tasa = ${fmt(f.row.v)} × 10.000 / ${fmt(r.population.population)} = ${fmt(f.rate)} por 10.000 habitantes<br>Máxima tasa comparable = ${fmt(f.anchor)}<br>z = ${f.rate===0?'0 (tasa cero explícita)':`100 × ${fmt(f.rate)} / ${fmt(f.anchor)}`} = ${fmt(f.score)}<br>Peso interno = ${fmt(f.share)}<br>Aporte al sector = ${fmt(f.score)} × ${fmt(f.share)} = ${fmt(f.score*f.share)}</div></div>`;
  }
  ...
  function render(model,state){
    ...
    get('radar-sectors').innerHTML=current.map((r,k)=>`<div><strong style="color:${colors[k]}">${esc(label(r))}</strong><div>${r.sectors.map((s,i)=>`<button type="button" class="secondary" data-radar-m="${k}" data-radar-axis="${i}">${esc(s.name)}: ${fmt(s.lower)}–${fmt(s.upper)}</button>`).join('')}</div></div>`).join('');
