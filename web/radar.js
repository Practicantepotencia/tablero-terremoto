...
(function(root){
  ...
  function relativeField(f,r){
    ...
    if(d)head+='<p>'+(f.estimated?'Base estimada ML: ':'Base: ')+'<strong>'+fmt(d.value)+' '+esc(d.unit)+'</strong> · '+esc(d.reference_date)+'<br><a href="'+esc(source.url)+'" target="_blank" rel="noopener">'+esc(source.label)+'</a><br><span class="small">'+esc(d.locator)+'</span></p>';
  ...
  function render(model,state){
    ...
    get('radar-legend').innerHTML=current.map((r,k)=>`<p style="color:${colors[k]}"><strong>${esc(label(r))}</strong> · P: ${fmt(r.lower)}–${fmt(r.upper)} · ${r.available}/${r.fieldCount} campos${r.mlScenario?' · '+r.imputedAvailable+' bases ML':''}${!r.coverage?' · Sin puntaje documentado':''}</p>`).join('');
