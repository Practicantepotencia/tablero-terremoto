...
(function(root){
  ...
  function relativeField(f,r){
    ...
    return head+'<div class="formula">Tasa = '+fmt(f.row.v)+' × '+f.multiplier+' / '+fmt(d.value)+' = '+fmt(f.rate)+' '+esc(f.relativeUnit)+(f.proxyNote?'<br>'+esc(f.proxyNote):'')+(f.normalization==='fixed_inventory_cap_1'?'<br>Tope fijo del cociente = 1<br>z = 100 × min('+fmt(f.rate)+', 1)':'<br>Máxima tasa comparable = '+fmt(f.anchor)+'<br>z = '+(f.rate===0?'0 (cero explícito)':'100 × '+fmt(f.rate)+' / '+fmt(f.anchor)))+' = '+fmt(f.score)+'<br>Peso interno = '+fmt(f.share)+'<br>Aporte al sector = '+fmt(f.score)+' × '+fmt(f.share)+' = '+fmt(f.score*f.share)+'</div></div>';
  }
  function inspect(index,axis){
    const r=current[index]; if(!r)return;
    active=[r.geo,axis];
    const adjustment=r.adjustment||Priorizacion.depthAdjustment('ipm',r.vulnerability,null),s=r.sectors[axis],loFactor=adjustment.factorLower,hiFactor=adjustment.factorUpper;
    get('radar-inspector').innerHTML=`<h3 style="color:${colors[index]}">${esc(label(r))}</h3><h4>${esc(s.name)}: ${relative&&!s.coverage?'Sin datos relativos':fmt(s.lower)+'–'+fmt(s.upper)+' / 100'}</h4>${relative?(percapita?'<p>Por 10.000 habitantes.</p>':'<p>Base propia de cada indicador.</p>'):''}${s.fields.map(f=>relative?relativeField(f,r):`<div class="radar-input"><strong>${esc(f.label)}</strong><div class="small muted">${esc(f.source)} · ${esc(f.unit)} · N = ${f.n}</div>${f.row?`<div>Valor = ${fmt(f.row.v)}; máximo = ${fmt(f.anchor)}</div><div class="formula">z = ${f.row.v===0?'0 (cero explícito)':`100 × ${fmt(f.row.v)} / ${fmt(f.anchor)}`} = ${fmt(f.score)}<br>Peso interno = ${fmt(f.share)}<br>Aporte al sector = ${fmt(f.score)} × ${fmt(f.share)} = ${fmt(f.score*f.share)}</div>`:`<p><strong>Sin dato, no cero.</strong> Máximo de referencia = ${fmt(f.anchor)}. Peso interno = ${fmt(f.share)}. Aporte desconocido al sector: 0–${fmt(100*f.share)}.</p>`}</div>`).join('')}<p class="formula">Sector inferior = ${s.fields.filter(f=>f.share>0).map(f=>f.score==null?'0 [límite, no dato]':fmt(f.score*f.share)).join(' + ')} = ${fmt(s.lower)}<br>Sector superior = ${fmt(s.lower)} + ${fmt(s.upper-s.lower)} por faltantes = ${fmt(s.upper)}</p><h4>Cómo entra al índice global</h4><p>D = (${r.sectors.map(x=>fmt(x.lower)).join(' + ')}) / ${r.sectors.length} = ${fmt(r.damageLower)} · documentado.</p><p>${esc(Priorizacion.adjustmentNote(r,fmt))}</p><p class="formula">${esc(adjustment.formula)}<br>P inferior = ${fmt(r.damageLower)} × ${fmt(loFactor)} = ${fmt(r.lower)}<br>P superior = ${fmt(r.damageUpper)} × ${fmt(hiFactor)} = ${fmt(r.upper)}</p><p>Aporte de este sector a P: ${fmt(loFactor==null?null:s.lower/r.sectors.length*loFactor)}–${fmt(hiFactor==null?null:s.upper/r.sectors.length*hiFactor)} puntos · peso 1/${r.sectors.length}.</p>`;
  }
  function render(model,state){
    ...
    get('radar-reference').textContent=`Modelo ${Priorizacion.VERSION}${relative?(percapita?' · Per cápita · población DANE ':' · Denominadores sectoriales · fecha del daño ')+String(state.date).slice(0,4):''} · Captura ${state.date} · Referencia: ${state.scope==='decree'?'departamentos del decreto':'todos los departamentos del inventario'}, ${p.referenceN} municipios.${mode==='sectorial'?' '+Priorizacion.DEPTHS[state.relativeDepth||'ipm']+'. Los ejes muestran los sectores antes del ajuste global.':''}`;
    ...
    current.forEach((r,k)=>{
      if(relative&&r.sectors.some(s=>!s.coverage)){
        ...
        r.sectors.forEach((s,i)=>{
          const next=(i+1)%r.sectors.length,t=r.sectors[next];if(!s.coverage||!t.coverage)return;
    ...
    get('radar-legend').innerHTML=current.map((r,k)=>`<p style="color:${colors[k]}"><strong>${esc(label(r))}</strong> · P: ${fmt(r.lower)}–${fmt(r.upper)} · ${r.available}/${r.fieldCount} campos${!r.coverage?' · Sin puntaje documentado':r.adjustment?.depth==='ipm_idf'&&!r.fiscal?' · Sin IDF válido':''}</p>`).join('');
