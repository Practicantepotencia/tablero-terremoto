...
(function(root){
  ...
  function detail(r,absolute){
    ...
    $('relative-detail').innerHTML='<div class="section-head"><h3>'+esc(r.m)+', '+esc(r.d)+' · cálculo relativo por indicador</h3><button type="button" class="secondary" id="relative-close">Cerrar ficha</button></div><p>Prioridad relativa documentada: '+(r.coverage?range(r.lower,r.upper):'Sin puntaje')+' · puesto '+(r.rank??'—')+'. Prioridad absoluta: '+points(absolute?.lower)+' · puesto '+(absolute?.rank??'—')+'. Cobertura '+r.available+'/17 campos ('+points(100*r.coverage)+'% del peso).</p><div class="table-scroll"><table>'+headers(['Dimensión / variable','Valor original','Denominador y fuente','Tasa / proporción','Máxima tasa / N','Puntaje variable','Peso interno','Aporte al sector'])+'<tbody>'+table+'</tbody></table></div><p class="formula">Tasa = valor original × factor de presentación / denominador propio<br>z relativo = 100 × tasa / máxima tasa comparable<br>Sector = Σ(z relativo × peso interno)<br>D inferior = ('+r.sectors.map(s=>fmt(s.lower)).join(' + ')+') / 6 = '+fmt(r.damageLower)+'<br>P relativo inferior = '+fmt(r.damageLower)+' × '+fmt(factorLo)+' = '+fmt(r.lower)+'<br>P relativo superior = '+fmt(r.damageUpper)+' × '+fmt(factorHi)+' = '+fmt(r.upper)+'</p><p>IPM DANE 2018: '+(r.vulnerability==null?'sin dato, se propaga 0–100':fmt(r.vulnerability)+'%')+'. Factor = (1 + 0,25 × IPM/100) / 1,25. Un cero explícito solo puntúa cero con denominador válido. Los campos desconocidos mantienen su peso: las sumas inferiores usan cero como límite, no como observación. El resultado parcial no permite afirmar que un municipio esté menos necesitado por carecer de datos.</p>';
  ...
  function render(state){
    ...
    $('relative-note').textContent=(sector?'Orden por '+sector.name+', '+(direction==='desc'?'mayor a menor':'menor a mayor'):'Orden por límite documentado de la prioridad relativa')+'. Pulsa una dimensión: mayor a menor → menor a mayor → orden normal. Personas: por 10.000 habitantes; vivienda: por 100 viviendas. Bases DANE '+year+'.';
    $('relative-formula').textContent='Cada variable: tasa = valor × factor / denominador propio; puntaje = 100 × tasa / máxima tasa comparable. Se conservan los 17 campos, los seis sectores (1/6 cada uno) y el ajuste IPM. Referencia: '+(state.scope==='decree'?'departamentos del decreto':'inventario completo')+', captura '+state.date+'. Buscar, ordenar o filtrar departamento no cambia esa referencia. No se sustituye un denominador ausente por población. Bases verificadas el '+(DATA.denominators?.checked?.slice(0,10)||'—')+'.';
    $('relative-count').textContent=result.items.length+' municipios visibles · '+result.items.filter(r=>r.coverage>0).length+' con algún puntaje relativo · '+result.referenceN+' en la referencia. Cada variable excluye numeradores y denominadores faltantes, incompatibles o no verificados.';
