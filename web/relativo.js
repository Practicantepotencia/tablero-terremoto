...
(function(root){
  ...
  function render(state){
    ...
    $('relative-note').textContent=(sector?'Orden por '+sector.name+', '+(direction==='desc'?'mayor a menor':'menor a mayor'):'Orden por límite documentado de la prioridad relativa')+'. Pulsa una dimensión: mayor a menor → menor a mayor → orden normal. '+(percapita?'Todos los conteos por 10.000 habitantes.':'Personas: por 10.000 habitantes; vivienda: por 100 viviendas.')+' Bases DANE '+year+'.';
    $('relative-formula').textContent=(percapita?'Cada variable: tasa = valor × 10.000 / población municipal; puntaje':'Cada variable: tasa = valor × factor / denominador propio; puntaje')+' = 100 × tasa / máxima tasa comparable. Se conservan los 17 campos, los seis sectores (1/6 cada uno) y el ajuste IPM. Referencia: '+(state.scope==='decree'?'departamentos del decreto':'inventario completo')+', captura '+state.date+'. Buscar, ordenar o filtrar departamento no cambia esa referencia. '+(percapita?'Mide concentración por residente, no porcentaje de instalaciones dañadas. No se imputa población ausente.':'No se sustituye un denominador ausente por población. Bases verificadas el '+(DATA.denominators?.checked?.slice(0,10)||'—')+'.');
