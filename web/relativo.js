...
(function(root){
  ...
  const scenario=()=>!percapita&&DATA.denominators?.health_variant?.ml?.enabled===true;
  const coverageLabel=r=>scenario()?'Cobertura sin ML '+points(100*r.observedCoverage)+'% · '+r.imputedAvailable+' bases ML · '+r.available+'/'+r.fieldCount+' calculados':'Cobertura '+points(100*r.coverage)+'% · '+r.available+'/'+r.fieldCount+' campos';
  ...
  const range=(lo,hi)=>Math.abs(lo-hi)<1e-8?points(lo)+' /100':(scenario()?'Escenario ':'Documentado ')+points(lo)+' · posible '+points(hi)+' /100';
  ...
  function detail(r,absolute){
    ...
    $('relative-detail').innerHTML='<div class="section-head"><h3>'+esc(r.m)+', '+esc(r.d)+(percapita?' · cálculo per cápita':' · cálculo relativo por indicador')+'</h3><button type="button" class="secondary" id="'+prefix+'-close">Cerrar ficha</button></div><p>'+(scenario()?'Prioridad relativa · escenario ML: ':'Prioridad relativa documentada: ')+(r.coverage?range(r.lower,r.upper):'Sin puntaje')+' · puesto '+(r.rank??'—')+'. Prioridad absoluta: '+points(absolute?.lower)+' · puesto '+(absolute?.rank??'—')+'. '+(scenario()?coverageLabel(r):'Cobertura '+r.available+'/'+r.fieldCount+' campos · '+points(100*r.coverage)+'% del peso')+'.</p><div class="table-scroll"><table>'+headers(['Dimensión / variable','Valor original','Denominador y fuente','Tasa / proporción','Máxima tasa / N','Puntaje variable','Peso interno','Aporte al sector'])+'<tbody>'+table+'</tbody></table></div><p class="formula">Tasa = valor original × factor de presentación / denominador propio<br>z relativo = 100 × tasa / máxima tasa comparable<br>Sector = Σ(z relativo × peso interno)<br>D inferior = ('+r.sectors.map(s=>fmt(s.lower)).join(' + ')+') / 6 = '+fmt(r.damageLower)+'<br>P relativo inferior = '+fmt(r.damageLower)+' × '+fmt(factorLo)+' = '+fmt(r.lower)+'<br>P relativo superior = '+fmt(r.damageUpper)+' × '+fmt(factorHi)+' = '+fmt(r.upper)+'</p><p>IPM DANE 2018: '+(r.vulnerability==null?'sin dato, se propaga 0–100':fmt(r.vulnerability)+'%')+'. Factor = (1 + 0,25 × IPM/100) / 1,25. </p>';
  ...
  function render(state){
    ...
    const sector=Priorizacion.sectorsFor(state.severity).find(s=>s.id===sectorId),year=String(state.date).slice(0,4);
    $('relative-note').textContent=(sector?'Orden por '+sector.name+', '+(direction==='desc'?'mayor a menor':'menor a mayor'):(scenario()?'Orden por prioridad del escenario ML':'Orden por prioridad documentada'))+'. '+(percapita?'Todos los conteos por 10.000 habitantes.':'Personas: por 10.000 habitantes; vivienda: por 100 viviendas.')+' DANE '+year+'.';
    if(!percapita&&DATA.denominators?.health_variant?.notice)$('relative-note').textContent+=' '+DATA.denominators.health_variant.notice;
    ...
    $('relative-matrix').querySelectorAll('thead th').forEach((th,i)=>{
      if(!i)return;const s=Priorizacion.sectorsFor(state.severity)[i-1],active=s.id===sectorId;
