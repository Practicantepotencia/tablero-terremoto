...
(function(root){
  ...
  function detail(r,absolute){
    ...
    const adjustment=r.adjustment||Priorizacion.depthAdjustment('ipm',r.vulnerability,null),factorLo=adjustment.factorLower,factorHi=adjustment.factorUpper;
    ...
    $('relative-detail').innerHTML='<div class="section-head"><h3>'+esc(r.m)+', '+esc(r.d)+(percapita?' · cálculo per cápita':' · cálculo relativo por indicador')+'</h3><button type="button" class="secondary" id="'+prefix+'-close">Cerrar ficha</button></div><p>Prioridad relativa documentada: '+(r.coverage&&Number.isFinite(r.lower)?range(r.lower,r.upper):'Sin puntaje')+' · puesto '+(r.rank??'—')+'. Prioridad absoluta: '+points(absolute?.lower)+' · puesto '+(absolute?.rank??'—')+'. Cobertura '+r.available+'/'+r.fieldCount+' campos · '+points(100*r.coverage)+'% del peso.</p><div class="table-scroll"><table>'+headers(['Dimensión / variable','Valor original','Denominador y fuente','Tasa / cociente',fixedEducation?'Referencia / N':'Máxima tasa / N','Puntaje variable','Peso interno','Aporte al sector'])+'<tbody>'+table+'</tbody></table></div><p class="formula">Tasa = valor original × factor de presentación / denominador propio<br>'+(fixedEducation?'Educación: z = 100 × min(cociente, 1)<br>Resto: ':'')+'z relativo = 100 × tasa / máxima tasa comparable<br>Sector = Σ(z relativo × peso interno)<br>D inferior = ('+r.sectors.map(s=>fmt(s.lower)).join(' + ')+') / '+r.sectors.length+' = '+fmt(r.damageLower)+'<br>'+esc(adjustment.formula)+'<br>P relativo inferior = '+fmt(r.damageLower)+' × '+fmt(factorLo)+' = '+fmt(r.lower)+'<br>P relativo superior = '+fmt(r.damageUpper)+' × '+fmt(factorHi)+' = '+fmt(r.upper)+'</p><p>'+esc(Priorizacion.adjustmentNote(r,fmt))+'</p>';
    $('relative-close').onclick=()=>{selectedGeo='';$('relative-detail').hidden=true;};
  }
  function fieldTile(f){
    const title=f.rate==null?f.reason:f.label+' · '+f.source+'. '+f.denominatorLabel+': '+fmt(f.denominator.value)+'. Puntaje '+fmt(f.score)+(f.normalization==='fixed_inventory_cap_1'?'. Tope fijo ':'. Máxima tasa ')+fmt(f.anchor)+'; N='+f.n+'. '+(f.proxyNote||'');
  ...
  function render(state){
    ...
    if(!percapita&&!DATA.healthPressure?.enabled&&DATA.denominators?.registry_proxies?.notice)$('relative-note').textContent+=' '+DATA.denominators.registry_proxies.notice;
